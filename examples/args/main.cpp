...
public:
    RCP<UtilityBase> createUtility() const {
...
class ClientA {
public:
    void f(const UtilityBase &utility) const {
        utility.f();
    }
};
...
public:
    void initialize(const RCP<UtilityBase> &utility) {
        utility_ = utility;
    }
    void g(const ClientA &a) {
        a.f(*utility_);
    }
};
...
public:
    ...
    void h(const Ptr<ClientB> &b) {
        if(shareUtility_)
            b->initialize(utility_);
        else
            b->initialize(utilityFactory_->createUtility());
    }
};
